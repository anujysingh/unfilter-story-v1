...
export default function News({ newsItems = [] }: { newsItems?: any[] }) {
  ...
  const breakingNewsDb = newsItems.length > 0 ? newsItems[0] : null;
  const breakingNews = {
    title: breakingNewsDb?.title || "Zepto raises $350M Series E at $5B valuation in India's biggest quick-commerce round",
    excerpt: breakingNewsDb?.content?.replace(/<[^>]*>/g, '').substring(0, 160) + "..." || "Quick commerce unicorn Zepto has secured $350M in Series E funding at a $5B valuation, making it one of the largest funding rounds in India's startup ecosystem this year.",
    timestamp: breakingNewsDb?.pubDate ? new Date(breakingNewsDb.pubDate).toLocaleDateString("en-US", { month: "short", day: "numeric" }) : "Breaking • 15 min ago",
    category: breakingNewsDb?.categories?.[0] || "Funding",
    thumbnail: "https://images.unsplash.com/photo-1556740758-90de374c12ad?w=800&h=400&fit=crop",
    link: breakingNewsDb?.link || "#"
  };
  ...
  const displayNewsItems = newsItems.length > 1 ? newsItems.slice(1).map((item, idx) => {
    // Basic category classification from title or categories
    let category = "News";
    if (item.categories && item.categories.length > 0) {
      category = item.categories[0];
    } else {
      const titleLower = item.title.toLowerCase();
      if (titleLower.includes("raise") || titleLower.includes("fund") || titleLower.includes("valuation") || titleLower.includes("$")) {
        category = "Funding";
      } else if (titleLower.includes("layoff") || titleLower.includes("fire") || titleLower.includes("cut job")) {
        category = "Layoffs";
      } else if (titleLower.includes("launch") || titleLower.includes("introduce") || titleLower.includes("unveil")) {
        category = "Product Launches";
      } else if (titleLower.includes("acquire") || titleLower.includes("merger") || titleLower.includes("buyout")) {
        category = "Acquisitions";
      } else if (titleLower.includes("policy") || titleLower.includes("rbi") || titleLower.includes("regulation")) {
        category = "Policy";
      }
    }
    ...
    return {
      id: `rss-${idx}`,
      title: item.title,
      timestamp: item.pubDate ? new Date(item.pubDate).toLocaleDateString("en-US", { month: "short", day: "numeric" }) : "Recent",
      category: category,
      thumbnail: `https://images.unsplash.com/photo-${1507679799987 + (idx % 10)}?w=400&h=300&fit=crop`,
      excerpt: item.content?.replace(/<[^>]*>/g, '').substring(0, 140) + "...",
      link: item.link
    };
  }) : defaultNewsItems;
  ...
  const filteredNewsItems = activeCategory === "All"
    ? displayNewsItems
    : displayNewsItems.filter(item => item.category.toLowerCase().includes(activeCategory.toLowerCase()) || activeCategory.toLowerCase().includes(item.category.toLowerCase()));
        ...
        <div className="max-w-7xl mx-auto px-6 py-2">
          <div className="flex items-center gap-3 text-white overflow-x-auto scrollbar-hide">
            ...
            <div className="border-l-2 border-white/30 pl-3 py-1">
              <a href={breakingNews.link} target="_blank" rel="noopener noreferrer" className="font-medium whitespace-nowrap hover:underline">
                {breakingNews.title}
              </a>
            </div>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-12">
        ...
        <div className="grid md:grid-cols-10 gap-8">
          <div className="md:col-span-7 space-y-6">
            <a href={breakingNews.link} target="_blank" rel="noopener noreferrer" className="block bg-gradient-to-br from-[#111111] to-[#2A2A2A] text-white rounded-2xl overflow-hidden hover:shadow-2xl hover:shadow-pink-500/20 transition-all border-2 border-transparent hover:border-[#FF4D8D]">
            ...
            {filteredNewsItems.length === 0 ? (
              <div className="text-center py-20 bg-gray-50 rounded-2xl border border-gray-200">
                <h3 className="text-2xl font-bold text-gray-500 mb-2">No news items found under this category.</h3>
                <p className="text-sm text-gray-400">Try choosing a different filter above.</p>
              </div>
            ) : (
              filteredNewsItems.map((news, i) => (
                <a
                  key={i}
                  href={news.link}
                  target="_blank"
                ...
                >
                  <div className="md:flex gap-6">
                    ...
                    <div className="p-6 flex-1 min-w-0">
                      ...
                      <h2 className="text-xl font-bold text-[#111111] mb-3 group-hover:text-[#FF4D8D] transition-colors leading-tight">
                        {news.title}
                      </h2>
