...
export default function ContactUs() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await fetch("http://localhost:3000/v1/contact", {
        method: "POST",
                ...
                <form onSubmit={handleSubmit} className="space-y-8">
                  <div className="grid md:grid-cols-2 gap-8">
                    <div className="space-y-3">
                      ...
                      <input required type="text" name="name" placeholder="John Doe" className="w-full px-6 py-5 bg-[#F8F9FA] border border-transparent rounded-2xl focus:outline-none focus:bg-white focus:border-[#FF4D8D] transition-all" />
                    </div>
                    <div className="space-y-3">
                      <label className="text-xs font-black text-[#111111] uppercase tracking-[0.2em] ml-1">Email Address</label>
                      <input required type="email" name="email" placeholder="john@company.com" className="w-full px-6 py-5 bg-[#F8F9FA] border border-transparent rounded-2xl focus:outline-none focus:bg-white focus:border-[#FF4D8D] transition-all" />
                    </div>
                  </div>

                  <div className="space-y-3">
                    <label className="text-xs font-black text-[#111111] uppercase tracking-[0.2em] ml-1">Subject</label>
                    <input required type="text" name="subject" placeholder="How can we help?" className="w-full px-6 py-5 bg-[#F8F9FA] border border-transparent rounded-2xl focus:outline-none focus:bg-white focus:border-[#FF4D8D] transition-all" />
                  </div>

                  <div className="space-y-3">
                    <label className="text-xs font-black text-[#111111] uppercase tracking-[0.2em] ml-1">Your Message</label>
                    <textarea required name="message" rows={8} placeholder="Write your message here..." className="w-full px-8 py-6 bg-[#F8F9FA] border border-transparent rounded-2xl focus:outline-none focus:bg-white focus:border-[#FF4D8D] transition-all resize-none" />
                  </div>
                  ...
                  <button 
                    type="submit" 
                    disabled={loading}
                    className={`w-full bg-gradient-to-r from-[#FF4D8D] to-[#FF7A18] text-white py-6 rounded-2xl font-black text-xl shadow-xl shadow-[#FF4D8D]/20 flex items-center justify-center gap-4 hover:shadow-2xl hover:shadow-[#FF4D8D]/30 transition-all hover:-translate-y-1 ${loading ? 'opacity-70 cursor-not-allowed' : ''}`}
                  >
