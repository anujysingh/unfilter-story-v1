...
export default function Header() {
  ...
  const navLinks = [
    { path: "/", label: "Home" },
    { path: "/startup-news", label: "Startup News" },
        ...
        <nav className="hidden md:flex items-center gap-8">
          {navLinks.map((link) => (
            <Link
              key={link.path}
              to={link.path}
              className={`relative transition-colors hover:text-[#FF4D8D] font-medium ${pathname === link.path
                  ? "text-[#111111]"
                  : "text-[#6B6B6B]"
                }`}
            >
          ...
          >
            <button className={`flex items-center gap-1 transition-colors hover:text-[#FF4D8D] font-medium ${moreLinks.some(link => pathname === link.path) ? "text-[#111111]" : "text-[#6B6B6B]"
              }`}>
              More
              ...
              <div className="bg-white border border-[#EAEAEA] rounded-2xl shadow-xl py-3 min-w-[200px] overflow-hidden">
                {moreLinks.map((link) => (
                  <Link
                    key={link.path}
                    to={link.path}
                    className={`block px-6 py-3 transition-colors hover:bg-[#F9F9F9] hover:text-[#FF4D8D] ${pathname === link.path ? "text-[#FF4D8D] bg-[#F9F9F9]" : "text-[#6B6B6B]"
                      }`}
                  >
                    {link.label}
                  </Link>
