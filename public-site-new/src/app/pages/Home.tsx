...
export default function Home() {
  ...
  const featuredStory = {
    id: "story-1",
    title: "The Day We Lost Everything: A $5M Startup Goes to Zero",
    description: "How poor unit economics, rapid expansion, and investor pressure led to a complete shutdown in 90 days.",
    industry: "SaaS",
    author: "Priya Sharma",
    date: "Apr 24, 2026",
    readTime: "15 min",
    ...
    <div className="bg-white">
      ...
      <section className="max-w-7xl mx-auto px-6 py-12">
        <div className="grid md:grid-cols-10 gap-8">
          <Link to={`/stories/${featuredStory.id}`} className="md:col-span-7 group cursor-pointer rounded-2xl overflow-hidden border border-[#EAEAEA] hover:border-[#FF7A18] transition-all h-full">
          ...
          <div className="md:col-span-3">
            <div className="sticky top-24 space-y-4">
              ...
              {quickFeed.map((item, i) => (
                <Link key={i} to={`/news/${item.id}`} className="flex gap-3 group cursor-pointer border-b border-[#F9F9F9] pb-4 last:border-0">
        ...
        <div className="flex flex-col md:flex-row gap-8">
          <div className="flex-1 grid md:grid-cols-3 gap-8">
            {filteredStories.map((story, i) => (
              <Link key={i} to={`/stories/${story.id}`} className="group cursor-pointer rounded-2xl overflow-hidden border border-[#EAEAEA] hover:border-[#FF7A18] hover:shadow-xl transition-all bg-white">
